...
for wind_mag in np.arange(0.4,3.8,0.2):
    ...
    file_name = 'trap_arrival_by_wind'
    file_name = file_name +'_wind_mag_'+str(wind_mag)
    file_name = 'disp_test_1'
    ...
    while t<simulation_time:
        for k in range(capture_interval):
            ...
            #update the swarm
            try:
                swarm.update(t,dt,wind_field,importedPlumes,traps,pre_stored=True) #for presaved plumes
            except(IOError):
                print('--------Trouble with plume data load-----')
                test_p = importedPlumes.value(t,3000*np.random.randn(20),
                    3000*np.random.randn(20))
                test_p = importedPlumes.value(t,swarm.x_position,swarm.y_position)
