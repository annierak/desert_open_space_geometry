...
def f2(intersection_distances,K,x_0):
...
#Convert intersection_distances to probabilities
success_probabilities = f2(intersection_distances,K,x_0)
# print('(flies x traps)')
# print(np.array2string(success_probabilities,precision=2))
...
x_0_slider = Slider(x_0_ax, 'x_0', 0., 600.0, valinit=300.)
...
def update(val):
    ...
    for i in range(8):
        ...
        #     xs,ys = patches[i].get_xy().T
        #     # print(patches[i].get_xy())
        #     old_bbox = patches[i].get_extents()
        #     print(old_bbox)
        ...
        #     # patches[i].set_clip_box(new_bbox)
        #     old_bbox.set_points([[x0, y0], [x1, y1]])
        #     new = patches[i].get_extents()
        #     print(new)
        #     print('--------')
        #     # patches[i].set_xy(np.vstack((xs,ys)).T)
        #     # raw_input()
        #     # patches[i].set_x(min(t_sim))
        #     # patches[i].set_width(max(t_sim)-min(t_sim))

            # patches[i].update(min(t_sim),max(t_sim)-min(t_sim))
...
stime = Slider(axtime, 'Time', 0., 20.0, valinit=1.)
